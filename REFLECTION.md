...
I used AI tools (specifically Antigravity and Claude 3.5) as a **Pair Programmer and Product Architect**. For coding, I used them to generate the boilerplate for the multi-step form and to help debug the Windows-specific CLI errors during the `shadcn/ui` setup. I didn't trust the AI with the **Audit Logic**—I manually researched the pricing data and wrote the deterministic TypeScript functions myself. This was a deliberate choice: AI is prone to "price hallucination," and for a finance tool, a 10% error in pricing destroys all credibility. 

A specific time the AI was wrong was during the Supabase schema design; it suggested a complex RLS (Row Level Security) policy that would have blocked public reads for the shareable links. I caught this during testing when the `/share` page returned a 403 error. I manually simplified the policy to allow `SELECT` on the `audits` table for all users while keeping `leads` strictly private.
